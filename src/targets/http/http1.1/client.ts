...
export const http11: Client<Http11Options> = {
  ...
  convert: ({ method, fullUrl, uriObj, httpVersion, allHeaders, postData }, options) => {
    ...
    // Convert LF to CRLF in message body to comply with RFC 7230
    let messageBody = postData.text || '';
    if (messageBody) {
      // Replace any existing CRLF with LF first to avoid double conversion
