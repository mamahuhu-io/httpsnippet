...
export class HTTPSnippet {
  ...
  constructor(input: HarEntry | HarRequest) {
    ...
    entries.forEach(({ request }) => {
      ...
      // in code generation, as used in the Auk app.
    ...
    switch (request?.postData.mimeType) {
      ...
      case 'multipart/alternative':
        ...
        if (request.postData?.params) {
          ...
          // Ensure multipart bodies always use CRLF line endings. Some environments
          // (or dependencies) may emit LF-only lines, which causes fixture diffs on CI.
          if (request.postData.text) {
            request.postData.text = request.postData.text.replace(/\r?\n/g, '\r\n');
          }
